...
public abstract class IdentityProviderDAO implements DaoBase<IdentityProvider>, Transactional<IdentityProviderDAO> {
    ...
    @SqlQuery(" SELECT "
            + " s.name AS name, "
            + " s.type AS type "
            + " FROM common_oltp.sso_login_provider s "
            + " INNER JOIN "
            + " common_oltp.user_sso_login su "
            + " ON "
    ...
    @SqlQuery("SELECT "
            ...
            + "s.type AS type "
            + "FROM common_oltp.sso_login_provider s "
            + "INNER JOIN common_oltp.user_sso_login su ON su.provider_id = s.sso_login_provider_id "
            + "INNER JOIN common_oltp.user u ON u.user_id = su.user_id "
            + "WHERE "
    ...
    @SqlQuery(" SELECT "
            + " s.name AS name, "
            + " 'social' AS type "
            + " FROM common_oltp.social_login_provider s "
            + " INNER JOIN "
            + " common_oltp.user_social_login su "
            + " ON "

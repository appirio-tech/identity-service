...
aws s3api put-object --bucket "${AWS_S3_BUCKET}" --key "${AWS_S3_KEY}" --body ${WORK_DIR}/eb/${TAG}.zip
...
aws elasticbeanstalk create-application-version --application-name $APPNAME --version-label $APPVER --source-bundle S3Bucket="$AWS_S3_BUCKET",S3Key="$AWS_S3_KEY"
...
aws elasticbeanstalk update-environment --environment-name $AWS_EB_ENV --version-label $APPVER
handle_error "aws elasticbeanstalk update-environment failed."
